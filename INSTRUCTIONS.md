...
Your code goes in `src/`. Every function listed below is already there
with the right name and the right signature, and every one of them throws
"not implemented" until you replace that line with real code.

The tests are written for you, and they are RUNNING — so your first push
reports them as failures. That is expected, and it is your task list: each
failure names the function it is waiting for. Make them pass one at a time.

Do not edit the tests to make them pass. They are the specification, and
the same behaviour is checked again by tests you cannot see.
